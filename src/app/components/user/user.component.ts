...
export class UserComponent implements OnInit {
  ...
  editUser() {
    ...
    this.userService.editUser(row[0], row[0].email).subscribe((data) => {
  ...
  deleteUser() {
    ...
    if (selectedRows.length == 0) {
      this.notifier.notify(
        "error",
        "Please select atleat one User for deletion"
      );
      return;
    }
    this.userService
      .deleteUser(selectedRows[0].email)
  ...
  onCellEditingStopped(e) {
    ...
    this.api.forEachNode((node) => {
      if (!node.data.id) this.newData.push(node.data);
    });
